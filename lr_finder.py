...
class LRFinder(object):
    ...
    Arguments:
        ...
        criterion (torch.nn.Module): wrapped loss function. If provided 'None' then
            it will be assumed that loss will be directly provided by forward fn.
    ...
    def _train_batch(self, ele):
        # Set model to training mode
        self.model.train()
        recent_features = []
        for ele_r in ele['recent_features']:
            recent_features.append(ele_r.type(torch.FloatTensor).to(self.device))
        
        past_features = []
        for ele_p in ele['past_features']:
            past_features.append(ele_p.type(torch.FloatTensor).to(self.device))
        
        sentences = ele['sentences'].type(torch.LongTensor).to(self.device)
        
        labels = []
        for ele_l in ele['label']:
            labels.append(ele_l.type(torch.LongTensor).to(self.device)) # ele_l = [bs, max_prop, 2]
            
        current_divison_timings = []
        for cur_div in ele['current_divison_timings']:
            current_divison_timings.append(cur_div.type(torch.FloatTensor).to(self.device)) # current_divison_timings = [bs, 31]
            
        gt_truth_timings = ele['actual_timings'].type(torch.FloatTensor).to(self.device) # gt_truth_timings = [bs, max_prop, 2]
        duration = ele['duration'].type(torch.FloatTensor).to(self.device)

        # Forward pass
        self.optimizer.zero_grad()
        proposal_score, outputs, filter_used = self.model(sentences, past_features, recent_features, 
                                                          gt_truth_timings, True, 0.5, duration)  
        ...
        ground_truth_proposals_valid = torch.gt(gt_truth_timings_end, gt_truth_timings_start).squeeze().type(torch.FloatTensor).to(self.device)
        ...
        loss_start = self.criterion[0](predicted_start_proposal_flt_filtered, gt_truth_timings_start_filtered)
        loss_length = self.criterion[0](predicted_len_proposal_flt_filtered, gt_truth_timings_length_filtered)
        loss_valid_proposals = self.criterion[1](predicted_scores, ground_truth_proposals_valid)

        vocab_loss = torch.tensor(0, dtype=torch.float32).to(self.device)
        trg = None
        if outputs is not None:
            ...
            vocab_loss = self.criterion[2](outputs_flt,trg_flt) 

        total_loss = vocab_loss + loss_start + loss_length + loss_valid_proposals
    ...
    def plot(self, fname=None, to_show=False, skip_start=10, skip_end=5, log_lr=True):
        """Plots the learning rate range test.

        Arguments:
            fname (str) : file in which plot will be saved
                Default: None
            to_show (bool, optional) : whether to show the plot
                Default: False
