...
def main(args):
    ...
    if args.verbose:
        ...
        for i, tiou in enumerate(args.tious):
            ...
            for metric in evaluator.scores:
                ...
                if metric not in print_dict:
                    print_dict[metric] = [score]
                else:
                    print_dict[metric].append(score)
    ...
    for metric in evaluator.scores:
        ...
        print_dict[metric].append(avg_score)
        print('| %s: %2.4f' % (metric, 100 * sum(score) / float(len(score))))

    with open(args.out_file, 'a+') as fp:
        for ele in print_dict.keys():
            ele_to_print = ele + "," + ",".join(map(str, print_dict[ele])) + "\n"
            fp.write(ele_to_print)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate the results stored in a submissions file.')
    parser.add_argument('-s', '--submission', type=str, default='proposal_validation.json',
    ...
    parser.add_argument('-ppv', '--max-proposals-per-video', type=int, default=30,
    ...
    parser.add_argument('--out_file', type=str, default='results_outputs.csv', help="File where we print the scores")
    args = parser.parse_args()

    with open(args.out_file, 'w') as fp:
        header = ",".join(map(str, args.tious))
        fp.write("IOU," + header + ", AverageScores\n")
