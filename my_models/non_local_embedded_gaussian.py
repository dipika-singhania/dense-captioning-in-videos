...
class NONLocalBlock1D(nn.Module):
    def __init__(self, non_local_block_params, dim_curr, dim_past, linear_dim):
        super(NONLocalBlock1D, self).__init__()

        self.in_dim1 = dim_curr
        self.in_dim2 = dim_past
        self.linear_dim = linear_dim
        self.latent_dim =  non_local_block_params['latent_dim']

        self.scale = non_local_block_params['scale']
        self.scale_factor = non_local_block_params['scale_factor']
        self.dropout_rat = non_local_block_params['dropout_rate']
        ...
        if self.scale :
            self.scale_factor = nn.Parameter(torch.tensor([self.latent_dim**self.scale_factor],
                                              requires_grad=True))

        self.final_layers = nn.Sequential(
                            nn.LayerNorm(torch.Size([self.latent_dim, self.linear_dim])),
                            nn.ReLU(),
