...
class EncodingProposal(nn.Module):
    ...
    def forward(self, x_past_actual_all, x_curr_actual_all):
        ...
        for i, lstm_layer in enumerate(self.lstm_layer_list):
            ...
            encoded_features_list.append(tab_output_list[i])
            ...
            proposals = torch.softmax(self.v_list[i](tab_output_list[i]), dim=-1)  # Batch_size * seq len * max_proposals [4, 30, 27]

            proposals = proposals.permute(0, 2, 1)  # proposals = [4, 27, 31]

            # curr_start_enc = torch.matmul(proposals, encoding)
            curr_start_enc = torch.matmul(proposals, tab_output_list[i])
...
class CaptioningEncoderProposal(nn.Module):
    def __init__(self, max_proposals, linear_dim, encoding_proposal_params):
        ...
        self.attn_list = nn.ModuleList([nn.Linear(self.enc_dim, self.max_proposal, bias=False) \
                                        for _ in encoding_proposal_params['dim_cur_list']])
        self.encode_final_list = nn.ModuleList([nn.Sequential(\
                                                nn.LayerNorm(torch.Size([self.max_proposal, self.enc_dim])),
                                                nn.ReLU(),
                                                nn.Linear(self.enc_dim, self.enc_dim),
                                               ) for _ in encoding_proposal_params['dim_cur_list']])

        len_recent = len(encoding_proposal_params['dim_cur_list'])
        self.v = nn.Linear(self.enc_dim * len_recent, self.max_proposal, bias=False)
        self.v_linear = nn.Sequential(nn.LayerNorm(torch.Size([self.max_proposal, self.enc_dim * len_recent])),
                                      nn.ReLU(),
                                      nn.Linear(self.enc_dim * len_recent, self.enc_dim * len_recent))
...
class CaptioningDecoderProposal(nn.Module):
    def __init__(self, max_proposals, linear_dim, embed_dim, vocab_size, pad_idx, len_recent):
        ...
        self.rnn = nn.LSTM((self.enc_dim * len_recent) + embed_dim, (self.enc_dim * len_recent), batch_first=True)
        self.fc = nn.Linear((self.enc_dim * len_recent) + (self.enc_dim * len_recent) + embed_dim, vocab_size)
...
class OverallNetwork(nn.Module):
    ...
    def forward(self, input_sentences, x_past_actual_all, x_curr_actual_all, gt_truth_timings, is_train,
        ...
        if is_train:
            ...
            val_score = proposal_score[:, :, 2].squeeze()  # val_score = [bs, max_proposals]
